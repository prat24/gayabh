...
class GayabhStepper:
    ...
    def __call__(self, curr_x):
        """called by basinhopping to get next_pt step"""
...
def onestep_demo():
    ...
    np.random.seed(13)
    next_pt = stepper(curr_step)

    print(f"next point: {next_pt}")
    print(f"next function value: {objective_func(next_pt):.4f}")

    step_vec = next_pt - curr_step
